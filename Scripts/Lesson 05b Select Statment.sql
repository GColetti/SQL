...
--------------------------------------------------------------------------------
  ...
  SELECT department_name, manager_id+1  , location_id
    FROM departments;
    
    --Display lastname, salary of an employee and multiply the salary with their commision percentage
    SELECT last_name,salary, commission_pct,  salary * commission_pct+500 -(89000/8) *100 +700000 AS calculation
        FROM employees;
   -- ||
    SELECT last_name,salary, commission_pct,  salary * commission_pct as "CalculaTion new"
        FROM employees;
        
SELECT 'Hello',first_name  ||'is ' ||first_name ||' and the last name is '  || last_name AS "Full Name", employee_id, email, phone_number, hire_date
    FROM  employees;
   --display all departments in employees tables 
