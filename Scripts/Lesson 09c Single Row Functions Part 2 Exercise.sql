...
/* 1) Create a report that produces the following for each employee as one
      ...
      SELECT last_name || ' earns' || to_char(salary, '$9,999.99') || ' monthly but wants ' || to_char(3 * salary, '$9,999.99') AS "Dream_Salaries"
      FROM employees;
...
/* 2) Display each employee's last name, hire date, and first salary review date.
      ...
      TO_CHAR(TRUNC(ADD_MONTHS(hire_date, 12), 'IW'), 'fmDay') || ', the ' || TO_CHAR(TRUNC(ADD_MONTHS(hire_date, 12), 'IW'), 'ddspth') || ' of ' || TO_CHAR(TRUNC(ADD_MONTHS(hire_date, 12), 'IW'), 'fmMonth') || ', ' || TO_CHAR(TRUNC(ADD_MONTHS(hire_date, 12), 'IW'), 'YYYY') AS first_review
      FROM employees;
...
/* 3) Display the last name, hire date, and day of the week on which the 
      ...
      SELECT last_name, hire_date, TO_CHAR(hire_date, 'D') AS start_day 
      FROM employees
      ORDER BY start_day;
...
/* 4) Display employee last names, and salary multiplied by commission percent
	...
	If the employee does not earn a commission, show "No Commission". Label the column COMM. */
      SELECT last_name,
            ...
            ELSE TO_CHAR(salary * commission_pct, '$9,999.99') END AS COMM
            FROM employees
