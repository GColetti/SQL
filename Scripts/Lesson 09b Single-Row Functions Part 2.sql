...
--------------------------------------------------------------------------------
*/
--display the last 3 letters of employee's last name and the result should be in uppeer case


SELECT last_name, CONCAT(UPPER(SUBSTR(last_name,-3,3)), first_name)
    from employees;

SELECT salary, NVL(commission_pct,0), salary+(NVL(commission_pct,0)) as bonus
    FROM employees;


SELECT salary,commission_pct, NVL2(commission_pct,0,1), salary+(NVL2(commission_pct,0,1)) as bonus
    FROM employees;

SELECT TO_CHAR(SYSDATE, 'dd/mm/yyy')
    FROM dual;
    --DISPLAY NUMBER OF MONTHS EMPLOYEES BEEN WORKING HERE
    SELECT first_name, ROUND(MONTHS_BETWEEN(SYSDATE, HIRE_DATE))
        FROM employees;
    
    SELECT TO_DATE('02/04/022','dd/mm/yyy')
        from dual;
    
    SELECT TO_CHAR(SYSTIMESTAMP, 'HH:MI:sssssssss A.M.')
        FROM dual;
    
    SELECT hire_date,TO_CHAR(, 'HH')
        from employees;
